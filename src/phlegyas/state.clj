...
(defmacro state!
  ...
  `(let [state-update# (:update ~data)
     ...
     (if state-update#
       (cond
         (clojure.test/function? state-update#)
         (swap! ~'state state-update#)

         (and (list? state-update#) (list? (first state-update#)))
         (swap! ~'state (fn [x#] (loop [applicators# state-update#
                                       s# x#]
                                  (if (nil? (first applicators#))
                                    s#
                                    (recur (rest applicators#)
                                           (apply (ns-resolve ~*ns* (ffirst applicators#))
                                                  (cons s# (map (fn [x#] (ns-resolve ~*ns* x#)) (rest (first applicators#))))))))))

         :else
         (swap! ~'state (fn [x#] (apply (ns-resolve ~*ns* (first state-update#))
                                       (cons x# (map (fn [x#] (ns-resolve ~*ns* x#)) (rest state-update#))))))))
...
(defn-frame-binding Tversion
  ...
  (cond
    ...
    :else (state! {:update (assoc :msize max-message-size)
                   :reply {:version protocol-version
...
(defn-frame-binding Twalk
  ...
  (if (= (count frame-wnames) 0)
    (state! {:update '((add-fid frame-newfid frame-tag)
...
(defn-frame-binding Topen
  ...
  (let [role (fid->role frame-fid current-state)
    ...
    (if (not (permission-check stat role :oread))
      ...
      (state! {:update '(update-mapping frame-fid {:offset 0})
               :reply {:iounit (iounit!)
...
(defn-frame-binding Tcreate
  ...
  (let [new-stat (create-synthetic-file frame-name #'example-function-for-files)
    ...
    (state! {:update '((assoc-in [:fs-map fs-name :files file-path]
                                 (into new-stat {:qid-path file-path :parent parent-path}))
                       (update-in [:fs-map fs-name :files parent-path]
                                  (fn [x] (assoc x :children (conj (:children x) file-path)))))
    ...
    (case typ
      ...
      :dir (if (and
             ...
             (let [dirpaths (if (= frame-offset 0)        ; read directory from beginning...
               ...
               (state! {:update '(update-mapping frame-fid {:offset (+ frame-offset (count dir-data))
...
(defn-frame-binding Tclunk
  [frame connection]
  (state! {:update (fn [x] (into {:fids (dissoc (:fids x) frame-fid)
...
(defn state-handler
  ...
  (let [reply (((:frame frame) state-handlers) frame connection)]
    (log/debug "state:" @(:state connection))
    (log/debug "out:" reply)
    (s/put! out reply)
